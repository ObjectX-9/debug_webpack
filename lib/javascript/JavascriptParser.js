...
class JavascriptParser extends Parser {
	...
	// TODO-编译阶段8：解析JavaScript 文本到AST
	/**
	...
	static _parse(code, options) {
		...
		let threw = false;
		try {
			// TODO-编译阶段9：调用 acorn 将 JS 文本解析为AST，这个 const parser = AcornParser.extend(importAssertions);
		...
		if (threw && type === "auto") {
			...
			try {
				// TODO-编译阶段10：调用 acorn 将 JS 文本解析为AST，这个 const parser = AcornParser.extend(importAssertions);
				ast = /** @type {AnyNode} */ (parser.parse(code, parserOptions));
				threw = false;
			} catch (e) {
