...
// TODO-初始化其他：lib/WebpackOptionsApply.js 文件，内部根据 entry 配置决定注入 entry 相关的插件，包括：DllEntryPlugin、
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		// TODO-初始化8：根据 devtool 值判断后续用那个插件处理 sourcemap，可选值：EvalSourceMapDevToolPlugin、SourceMapDevToolPlugin、EvalDevToolModulePlugin
		if (options.devtool) {
		...
		// TODO-初始化7：注入 EntryOptionPlugin 插件，处理 entry 配置
		new EntryOptionPlugin().apply(compiler);
		compiler.hooks.entryOption.call(options.context, options.entry);

		// TODO-初始化9：注入 RuntimePlugin ，用于根据代码内容动态注入 webpack 运行时
