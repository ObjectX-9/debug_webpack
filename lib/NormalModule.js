...
class NormalModule extends Module {
	...
	_doBuild(options, compilation, resolver, fs, hooks, callback) {
		...
		// TODO-编译阶段7：调用 loader-runner 仓库的 runLoaders 转译 module 内容，通常是从各类资源类型转译为 JavaScript 文本
		runLoaders(
			{
			...
			(err, result) => {
				...
				const loaderName = this.loaders.map(item=> {
					return item.loader;
				})
				console.log("✅ ✅ ✅ ~  loaders:", loaderName);
	...
	build(options, compilation, resolver, fs, callback) {
		...
		return this._doBuild(options, compilation, resolver, fs, hooks, err => {
			...
			// TODO-编译阶段12：AST 遍历完毕后，调用 module.handleParseResult 处理模块依赖
			handleParseResult();
		});
	}
