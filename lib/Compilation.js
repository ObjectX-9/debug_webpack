...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	addModuleTree({ context, dependency, contextInfo }, callback) {
		...
		if (!moduleFactory) {
			...
			// TODO-编译阶段3：正式开始编译阶段，调用 handleModuleCreate ，根据文件类型构建 module 子类
	...
	seal(callback) {
		...
		// TODO-生成阶段3：构建本次编译的 ChunkGraph 对象；
		...
		// TODO-生成阶段4：optimizeDependencies
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		// TODO-生成阶段7：optimizeModules
		while (this.hooks.optimizeModules.call(this.modules)) {
			/* empty */
		}
		this.hooks.afterOptimizeModules.call(this.modules);
		// TODO-生成阶段8：optimizeChunks
		while (this.hooks.optimizeChunks.call(this.chunks, this.chunkGroups)) {
			/* empty */
		}
		this.hooks.afterOptimizeChunks.call(this.chunks, this.chunkGroups);
		// TODO-生成阶段9：optimizeTree
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			// TODO-生成阶段10：optimizeChunkModules
				...
				err => {
					...
					this.codeGeneration(err => {
						...
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							// TODO-生成阶段12：createModuleAssets
